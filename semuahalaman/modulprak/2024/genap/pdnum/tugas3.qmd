...
3. *File-file* yang harus diunggah terdiri dari:
    ...
    c. Satu *file* .pdf (format penamaan: "penjelasan.pdf") yang berisi penjelasan terkait jawaban kalian, sesuai permintaan soal. Pembuatannya bebas apakah menggunakan Word, LaTeX, atau yang lainnya.
...
6. Dilarang dengan keras melakukan plagiarisme, baik mencontek ataupun dicontek. Jika terdapat mahasisya yang terindikasi melakukan plagiarisme, maka mahasiswa tersebut memperoleh nilai 0 untuk tugas ini. Namun, Anda sangat dianjurkan memanfaatkan apapun yang ada di [modul praktikum](./pdnum2024genap.qmd).
...
1. Diberikan masalah persamaan *transport* (adveksi):
    ...
    $$u(2,t) = e^{-4}, \quad 0 < t < 3$$

    yang memiliki solusi eksak $u(x,t) = e^{-x^2}$.
    ...
    b. Buatlah *script file* `soal_1b.m` berisi penggunaan metode Courant-Issacson-Rees untuk mengaproksimasi solusi dari PDP tersebut. Lampirkan gambar grafiknya (termasuk grafik solusi analitik) di *file* penjelasan. Seberapa mirip hasilnya dengan solusi analitik?

    c. Buatlah *script file* `soal_1c.m` berisi penggunaan metode Richardson untuk mengaproksimasi solusi dari PDP tersebut. Lampirkan gambar grafiknya di *file* penjelasan. Apakah solusi numeriknya stabil?

    d. Buatlah *script file* `soal_1d.m` berisi penggunaan metode Lax untuk mengaproksimasi solusi dari PDP tersebut. Lampirkan gambar grafiknya di *file* penjelasan. Seberapa mirip hasilnya dengan solusi analitik?

2. Diberikan masalah nilai awal-batas

    $$\begin{aligned*}
        u_t &= 3u_{xx}, \quad 0 \le x \le \pi, \quad 0 < t < T, \\
        u(0,t) &= u(\pi, t) = 0, \quad 0 < t < T, \\
        u(x,t) &= 3\sin{2x} - 6\sin{5x}, \quad 0 \le x \le \pi
    \end{aligned*}$$
    ...
    yaitu tentukan nilai $\alpha^2$, nilai $\text{xb}$, nilai $\text{xu}$, nilai $\text{tb}$, nilai $\text{tu}$, fungsi $f(x)$, fungsi $\text{lb}(t)$, dan fungsi $\text{lb}(t)$.
...
3. Diberikan persamaan gelombang
    ...
    yaitu tentukan nilai $\alpha^2$, nilai $\text{xb}$, nilai $\text{xu}$, nilai $\text{tb}$, nilai $\text{tu}$, fungsi $f(x)$, fungsi $g(x)$, fungsi $\text{lb}(t)$, dan fungsi $\text{lb}(t)$.
    ...
    $$\begin{tabular}{r|r|r|r|r|r}
        $t = 1$     & 0     & -0.7071 & -1.0000 & -0.7071 & 0 \\
        \hline
        $t = 0.75$  & 0     & -0.5000 & -0.7071 & -0.5000 & 0 \\
        \hline
        $t = 0.5$   & 0     &       0 &       0 & -0.0000 & 0 \\
        \hline
        $t = 0.25$  & 0     &  0.5000 &  0.7071 &  0.5000 & 0 \\
        \hline
        $t = 0$     & 0     &  0.7071 &  1.0000 &  0.7071 & 0.0000 \\
        \hline
