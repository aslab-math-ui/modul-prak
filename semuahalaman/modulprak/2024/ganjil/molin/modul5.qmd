...
Kita akan membahas pipe lebih lanjut di Bagian 3.4, tetapi secara singkat, pipe mengambil nilai di sebelah kirinya dan meneruskannya ke fungsi di sebelah kanannya. Contohnya:
...
Selain itu, ada juga `distinct()`, yang digunakan untuk mencari baris dengan nilai yang unik. Berbeda dengan `arrange()` dan `filter()`, fungsi ini juga dapat, jika diperlukan, mengubah kolom.
...
Perhatikan bahwa karena kami belum menetapkan hasil perhitungan di atas kembali ke `flights`, variabel baru `gain`, `hours`, dan `gain_per_hour` hanya akan dicetak tetapi tidak akan disimpan dalam bingkai data. Dan jika kita ingin hasilnya tersedia dalam bingkai data untuk penggunaan di masa mendatang, kita harus memikirkan dengan cermat apakah kita ingin hasilnya ditetapkan kembali ke `flights`, menimpa bingkai data asli dengan lebih banyak variabel, atau ke objek baru. Seringkali, jawaban yang benar adalah objek baru yang diberi nama secara informatif untuk menunjukkan isinya, mis., `delay_gain`, tetapi Anda mungkin juga memiliki alasan bagus untuk menimpa `flights`.
...
flights |> 
  select(!year:day)
```
Secara historis, operasi ini dilakukan dengan `-` alih-alih `!`, jadi kemungkinan besar Anda akan melihatnya di alam liar. Kedua operator ini memiliki tujuan yang sama tetapi dengan perbedaan perilaku yang tidak kentara. Kami merekomendasikan penggunaan `!` karena terbaca sebagai "no" dan berpadu dengan baik dengan `&` dan `|`.
...
Anda juga dapat menentukan di mana harus meletakkannya menggunakan `.before` dan `.setelah`.after` argumen, seperti di `mutate()`:
```{r}
flights |> 
...
Apa yang akan terjadi jika kita tidak memiliki pipa? Kita dapat menyarangkan setiap pemanggilan fungsi di dalam pemanggilan sebelumnya:
...
RStudio juga memungkinkan kamu untuk menggunakan operator pipe lainnya, yaitu `|>`, yang diperkenalkan dalam R 4.1.0. Untuk menggunakan operator pipe ini, kamu perlu mengubah pengaturan di RStudio agar `|>` menjadi default, bukan `%>%`. 
...
daily_flights <- daily |> 
  summarize(n = n())
```
Jika Anda senang dengan perilaku ini, Anda dapat memintanya secara eksplisit untuk menyembunyikan pesan tersebut:
```{r}
daily_flights <- daily |> 
  summarize(
    n = n(), 
...
`dplyr` dan `tidyverse` merupakan packages yang powerfull untuk
