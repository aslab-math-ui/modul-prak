...
Mirip, tetapi menggunakan `geom_point` daripada `geom_line`
...
```{r}
ggplot(df3, aes(x = factor(cyl), fill = "red")) + 
  geom_bar()
```

## *Pie Chart*

Sayangnya, di `ggplot2` tidak ada fungsi khusus untuk *pie chart*. Namun, kita bisa akali dengan membuat *bar chart* yang kemudian diterapkan koordinat polar
...
Di `ggplot2`, *histogram* juga seolah-olah seperti sejenis *bar chart*
...
```{r}
df5 <- data.frame(category = c("A", "B", "C"), value = c(20, 15, 25))
```

```{r}
ggplot(df5, aes(x = category, y = value)) + 
  geom_col()
```

# Lampiran untuk belajar lebih lanjut

ggplot2: <https://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html>
