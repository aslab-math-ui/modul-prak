...
Tambahan: apabila kita ingin menuliskan sekumpulan data, di R ada konsep *vector*. Kita bisa menggabungkan sejumlah data agar menjadi satu kesatuan dengan fungsi `c()` atau *combine*, misalnya sebagai berikut:
...
Ada baiknya, *file* `.csv` dan/atau *file* `.xlsx` yang ingin di-*import* diletakkan di *folder* yang sama dengan *file* `.R` yang sedang kalian gunakan. Dengan demikian, untuk meng-*import* `Iris.csv`, kalian tinggal mengetik
...
Misalnya, untuk kolom `Sepal.Length`, berikut ukuran pemusatan data:
...
Berikut ukuran persebaran data:
...
Untuk nilai modus, gunakan fungsi `Mode` dari *package* `DescTools`, yang tentunya perlu diinstal terlebih dahulu kalau belum, lalu diaktifkan:
