...
```{r}
x_norm <- seq(-4, 4, length = 100)
```

```{r}
plot(x_norm,
     dnorm(x_norm, mean = 0, sd = 1),
     type = 'o')
```
...
```{r}
plot(x_norm,
     pnorm(x_norm, mean = 0, sd = 1),
     type = 'o')
```

Contoh:

Misalkan $X$ merupakan variabel acak yang berdistribusi normal dengan $\mu = 50$ dan variansi $\sigma = 10$. Hitunglah
...
$$\begin{align*}
    \text{Pr}(45 < X < 62) &= \text{Pr}\left(\frac{45-50}{10} < \frac{X-50}{10} < \frac{62-50}{10}\right) \\
    &= \text{Pr}\left(\frac{45-50}{10} < Z < \frac{62-50}{10}\right) \\
    &= \text{Pr}(Z \leq \frac{62-50}{10}) \\
    &- \text{Pr}(Z \leq \frac{45-50}{10})
\end{align*}$$
