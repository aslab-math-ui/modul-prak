...
-   Sampel acak saling bebas/independen dan berukuran sama antar populasi
...
Selanjutnya, akan dibuat dataframe dari 30 sampel acak tersebut. Kolom pop.source menyatakan asal populasi (1,2,3) dan kolom pops.value menyatakan nilai sampel acak.
...
-   $F_\text{hitung} < F_{\alpha;(k-1);(N-k)}\$
-   *p-value* \< $\alpha$.
...
Sampel acak tersebut berasal dari suatu populasi dengan ruang nilai (1,2,...,9,10). Dapatkah kita menyimpulkan bahwa populasi tersebut berdistribusi uniform?
...
Terdapat pesan "Warning: Chi-squared approximation may be incorrect". Hal ini terjadi karena terdapat beberapa pengulangan nilai pada ruang sampel, di mana masing-masing nilai tersebut dianggap sebagai observasi independen yang memiliki probabilitas $\frac{1}{20}$. Sedangkan, kita ingin menguji apakah peluang masing-masing mata dadu muncul bernilai $\frac{1}{6}$.
...
```{r}
data.frame(lower_bound, upper_bound, obs_class,ex_class)
```
...
```{r}
chi_hitung <- sum((obs_class-ex_class)**2/ex_class)
chi_hitung

qchisq(p=1-alpha, df=7-1)
```

Karena $\chi^2_\text{hitung} = 9.108018 < 12.59159 = \chi^2_{\alpha, k-1}$, maka hasil uji berada di luar daerah kritis. Dengan kata lain, tidak terdapat bukti yang cukup untuk menolak $H_0$ di mana populasi berdistribusi $N(\mu=3.5, \sigma^2 = 0.7^2)$.
...
Ingat hipotesis yang akan diuji adalah: - $H_0$: treatment dan improvement tidak saling mempengaruhi (independen) - $H_1$: treatment dan improvement saling mempengaruhi (tidak independen)

Dengan tingkat signfikasi $\alpha=0.05$, daerah kritis diberikan oleh - $\chi^2_\text{hitung} > \chi^2_{\alpha, (r-1)(c-1)}$ - *p-value* \< $\alpha$.
...
hipotesis yang akan diuji adalah: - $H_0$: carb dan cyl tidak saling mempengaruhi (independen) - $H_1$: carb dan cyl saling mempengaruhi (tidak independen)
...
Silakan unduh worksheet berikut untuk melakukan uji hipotesis di Excel yang telah kita bahas.

[Worksheet Pertemuan 9 (Revisi)](./worksheet_pertemuan9.xlsx)
