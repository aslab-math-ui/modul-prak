...
Memanfaatkan Dalil 1, kita bisa anggap rata-ratanya sebagai rata-rata dari suatu distribusi normal, dan variansinya dibagi $n$ menjadi variansi dari distribusi normal tersebut, yang akan menghampiri distribusi *sample mean*
...
Sekarang kita hitung probabilitasnya menggunakan distribusi normal tersebut
...
```{r}
n3 <- 36
standard_error <- xbar3_sd / sqrt(n3)
xbar3_lower <- 3.85
xbar3_upper <- 4.45
z3_lower <- (xbar3_lower - xbar3_mean) / standard_error
z3_upper <- (xbar3_upper - xbar3_mean) / standard_error
prob3_z <- pnorm(z3_upper) - pnorm(z3_lower)
prob3_z
```

Bagaimanapun juga, walaupun kita malah menghitung probabilitas menggunakan distribusi normal tersebut, kira-kira segitulah probabilitasnya di distribusi *sample mean* yang sebenarnya kita inginkan. Itulah manfaat dari Dalil 1.

## Dalil 2: Tanpa Pengembalian

**Lalu bagaimana dengan tanpa pengembalian?** Misalkan kita punya suatu populasi
...
```{r}
hist(pop4, main = "Distribusi Uniform Diskrit", xlab = " ")
```
...
Dapat terlihat juga bahwa bentuk distribusinya mirip dengan distribusi normal! Akan tetapi, rumus rata-rata dan variansi untuk distribusi normal tersebut sedikit berbeda (karena kali ini **tanpa pengembalian**), lihat Dalil 2 berikut!

**Dalil 2**

Apabila sampel acak berukuran n diambil secara acak **tanpa pengembalian** dari suatu populasi berhingga berukuran $N$, dimana populasi tersebut memiliki mean $\mu$ dan variansi $\sigma^2$, maka distribusi probabilitas $\bar{X}$ akan menghampiri normal dengan $\mu_\bar{X} = \mu$ dan $\sigma^2 = \frac{\sigma^2(N-n)}{n(N-1)}$
