...
Sejauh ini, semua program yang kita buat melibatkan *command line* (juga disebut *command prompt* atau *terminal*), karena perlu menampilkan *output* dan menerima input. Program semacam itu disebut ***command line interface* (CLI)**.

Mungkin dalam kehidupan sehari-hari, kalian lebih terbiasa dengan aplikasi yang memiliki semacam tampilan dengan tombol-tombol yang bisa ditekan dan sebagainya. Tampilan seperti itu disebut ***graphical user interface* (GUI)**.
